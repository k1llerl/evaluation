...
class StrategyEvaluator:
    def __init__(self, rets, bench_rets, leverage=0.8, excess_method="minus_sum"):
        """
    ...
    def get_result(self):
        ...
        if self.result is None:
            self.result = {"return": self.get_return(False), "sharpe": self.get_sharpe(False),
            ...
            ex_result = {"ex_return": self.get_return(True), "ex_sharpe": self.get_sharpe(True),
    ...
    def get_result_by_year(self):
        ...
        # 存储结果
        results = pd.DataFrame([], index=["return", "sharpe", "maxDrawDown", "startDrawDown", "endDrawDown",
                                          "ex_return", "ex_sharpe", "ex_maxDrawDown",
    ...
    def save_result(self, path):
        ...
        self.result.to_excel(os.path.join(path, "result.xlsx"))
        self.year_result.to_excel(os.path.join(path, "result_by_year.xlsx"))
    ...
    def plot(self, path=None):
        ...
        data = [
            ['', 'anReturn', 'anSharpe', 'maxDD', 'maxDDStart', 'maxDDEnd'],
            ['absolute', f'{data["return"] * 100:.2f}%', f'{data["sharpe"]}',
             f'{data["maxDrawDown"] * 100:.2f}%',
             data["startDrawDown"], data["endDrawDown"]],
            ['excess', f'{data["ex_return"] * 100:.2f}%', f'{data["ex_sharpe"]}',
        ...
        ax3.set_xlabel('time')
        ax3.set_ylabel('PnL')
        ax3.plot(return_cums[return_cums.columns[0]], label=return_cums.columns[0], color="#D32F2F")
        ax3.plot(return_cums[return_cums.columns[1]], label=return_cums.columns[1], color="#1976D2")
        ...
        plt.tight_layout()
        if path is not None:
            plt.savefig(os.path.join(path, "strategy_evaluation.png"))
